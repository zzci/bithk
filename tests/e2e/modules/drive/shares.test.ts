// Drive sharing over the live API via the unified share module: direct shares
// (act as the recipient to prove the capability grant), public links (unauth
// metadata + password download + exhaustion), and the sent / received / links
// inboxes. Drive entries are `resource_type='drive_entry'` shares managed via
// `/api/shares/drive_entry/:id`; public access is served via `/api/shared/:token`.
...
interface PublicMeta { token: string; permission: string; requiresPassword: boolean; expired: boolean; exhausted: boolean }
...
describe("drive direct shares (cross-user grant)", () => {
  it("admin shares a file with the user, who can then read + download until revoked", async () => {
    ...
    // Before the share the user has no relationship → fail-closed 404.
    expect((await user.raw(`/api/drive/entries/${file.id}`)).status).toBe(404);

    // Admin grants a direct download share.
    const share = await admin.json<{ data: Share }>(`/api/shares/drive_entry/${file.id}`, {
      method: "POST",
    ...
    const entryShares = await admin.json<{ data: Share[] }>(`/api/shares/drive_entry/${file.id}`);
    ...
    const received = await user.json<{ data: Share[] }>("/api/shares/received");
    expect(received.data.find(s => s.id === share.data.id)).toBeDefined();
    const sent = await admin.json<{ data: Share[] }>("/api/shares/sent");
    expect(sent.data.find(s => s.id === share.data.id)).toBeDefined();

    // Revoke → the user loses the relationship and reads fail-closed 404 again.
    expect((await admin.raw(`/api/shares/${share.data.id}`, { method: "DELETE" })).status).toBe(200);
    expect((await user.raw(`/api/drive/entries/${file.id}`)).status).toBe(404);
...
describe("drive public links", () => {
  it("create → unauth metadata → password download → exhaustion → revoke", async () => {
    ...
    const links = await admin.json<{ data: Share[] }>("/api/shares/links");
    ...
    const meta = await anon.json<{ data: PublicMeta }>(`/api/shared/${token}`);
    expect(meta.data.requiresPassword).toBe(true);
    expect(meta.data.expired).toBe(false);

    // Downloading without / with the wrong password is rejected.
    expect((await anon.raw(`/api/shared/${token}/download`, { method: "POST", body: {} })).status).toBe(403);
    expect((await anon.raw(`/api/shared/${token}/download`, { method: "POST", body: { password: "nope" } })).status).toBe(403);

    // Correct password streams the bytes (consuming the only download).
    const ok = await anon.raw(`/api/shared/${token}/download`, { method: "POST", body: { password: "letmein" } });
    expect(ok.status).toBe(200);
    expect(await ok.text()).toBe("public bytes");

    // The single download is now exhausted → 410.
    const exhausted = await anon.raw(`/api/shared/${token}/download`, { method: "POST", body: { password: "letmein" } });
    expect(exhausted.status).toBe(410);

    // Revoke → the link 404s entirely.
    await admin.raw(`/api/shares/${share.data.id}`, { method: "DELETE" });
    expect((await anon.raw(`/api/shared/${token}`)).status).toBe(404);
  ...
  it("an expired link is reported and refuses download (410)", async () => {
    ...
    const res = await anon.raw(`/api/shared/${token}/download`, { method: "POST", body: {} });
  ...
  it("the owner can update a link (permission + activation)", async () => {
    ...
    const updated = await admin.json<{ data: Share }>(`/api/shares/${share.data.id}`, {
      method: "PATCH",
