import type { ProtectedEnv } from "@/shared/lib/types";
import { Hono } from "hono";
import { z } from "zod";
import { auditFromCtx } from "@/modules/audit/audit.context";
import { AppError, NotFoundError } from "@/shared/lib/errors";
import { describeRoute, errorJson, okJson, onValidationFailure, validator } from "@/shared/lib/openapi";
import { adminRequired, authRequired } from "@/shared/middleware/auth";
import {
  deleteSetting,
  getSetting,
  getSettings,
  isSensitiveKey,
  MASKED_VALUE,
  maskSensitiveValue,
  maskValue,
  setSetting,
} from "./settings.service";

// Case-insensitive so camelCase keys (e.g. `project.defaults.coverReferenceId`)
// are writable through the generic settings CRUD.
const SETTING_KEY_RE = /^[a-z0-9][\w.-]{0,127}$/i;
const LIKE_SPECIAL_RE = /[%_]/g;

// Upper bound (64 KiB) on a stored setting value. The column is TEXT NOT NULL,
// so without this an admin could persist an unbounded blob that every
// `GET /settings` then reads into memory.
const MAX_SETTING_VALUE_LENGTH = 64 * 1024;

const putSettingSchema = z.object({
  value: z.string().min(1).max(MAX_SETTING_VALUE_LENGTH),
});

// `prefix` is an optional LIKE filter; key-format validation stays a 404 (not a
// 422), so the path param is documented only as a free string here and the
// strict `SETTING_KEY_RE` check remains in `validateSettingKey`.
const listSettingsQuerySchema = z.object({ prefix: z.string().optional() });
const keyParamSchema = z.object({ key: z.string() });

const settingRowSchema = z.object({
  key: z.string(),
  value: z.string(),
  updatedBy: z.string().nullable(),
  updatedAt: z.string(),
});
const settingValueSchema = z.object({ key: z.string(), value: z.string() });

function validateSettingKey(key: string): void {
  if (!SETTING_KEY_RE.test(key)) {
    throw new NotFoundError("Setting", key);
  }
}

export function settingsRoutes() {
  const router = new Hono<ProtectedEnv>();

  router.use("*", authRequired);

  // GET /settings — list all settings
  router.get(
    "/settings",
    describeRoute({
      tags: ["infra1"],
      summary: "List settings",
      responses: {
        200: okJson(z.array(settingRowSchema)),
        401: { description: "Unauthenticated", ...errorJson },
        403: { description: "Admin only", ...errorJson },
      },
    }),
    adminRequired,
    validator("query", listSettingsQuerySchema, onValidationFailure),
    async (c) => {
      const db = c.get("db");
      const rawPrefix = c.req.valid("query").prefix;
      const prefix = rawPrefix ? rawPrefix.replace(LIKE_SPECIAL_RE, "\\$&") : undefined;
      const data = (await getSettings(db, prefix)).map(maskSensitiveValue);
      return c.json({ success: true, data });
    },
  );

  // ─── Generic CRUD (wildcard key) ───

  // GET /settings/:key — single setting
  router.get(
    "/settings/:key",
    describeRoute({
      tags: ["infra1"],
      summary: "Get a setting",
      responses: {
        200: okJson(settingValueSchema),
        401: { description: "Unauthenticated", ...errorJson },
        403: { description: "Admin only", ...errorJson },
        404: { description: "Not found", ...errorJson },
      },
    }),
    adminRequired,
    validator("param", keyParamSchema, onValidationFailure),
    async (c) => {
      const db = c.get("db");
      const { key } = c.req.valid("param");
      validateSettingKey(key);
      const value = await getSetting(db, key);
      if (value === null) {
        throw new NotFoundError("Setting", key);
      }
      return c.json({ success: true, data: { key, value: maskValue(key, value) } });
    },
  );

  // PUT /settings/:key — create or update
  router.put(
    "/settings/:key",
    describeRoute({
      tags: ["infra1"],
      summary: "Create or update a setting",
      responses: {
        200: okJson(z.null()),
        400: { description: "Masked placeholder rejected", ...errorJson },
        401: { description: "Unauthenticated", ...errorJson },
        403: { description: "Admin only", ...errorJson },
        404: { description: "Invalid key", ...errorJson },
        422: { description: "Validation error", ...errorJson },
      },
    }),
    adminRequired,
    validator("param", keyParamSchema, onValidationFailure),
    validator("json", putSettingSchema, onValidationFailure),
    async (c) => {
      const db = c.get("db");
      const { key } = c.req.valid("param");
      validateSettingKey(key);
      const body = c.req.valid("json");
      const user = c.get("user");

      // Reject saving the masked placeholder for sensitive keys
      if (isSensitiveKey(key) && body.value === MASKED_VALUE) {
        throw new AppError("Cannot save masked placeholder as value", 400, "MASKED_VALUE_REJECTED");
      }

      // Capture previous value BEFORE the update so the audit row preserves
      // the change history. Sensitive keys still get masked so the audit log
      // never carries a plaintext OAuth secret etc.
      const previousRaw = await getSetting(db, key);
      const previousValue = previousRaw === null
        ? null
        : maskValue(key, previousRaw);

      await setSetting(db, key, body.value, { updatedBy: user.id });

      await auditFromCtx(c, {
        action: "setting.updated",
        resourceType: "setting",
        resourceId: key,
        resourceName: key,
        detail: { previousValue, newValue: maskValue(key, body.value) },
        result: "success",
      });

      return c.json({ success: true, data: null });
    },
  );

  // DELETE /settings/:key
  router.delete(
    "/settings/:key",
    describeRoute({
      tags: ["infra1"],
      summary: "Delete a setting",
      responses: {
        200: okJson(z.null()),
        401: { description: "Unauthenticated", ...errorJson },
        403: { description: "Admin only", ...errorJson },
        404: { description: "Not found", ...errorJson },
      },
    }),
    adminRequired,
    validator("param", keyParamSchema, onValidationFailure),
    async (c) => {
      const db = c.get("db");
      const { key } = c.req.valid("param");
      validateSettingKey(key);

      const deleted = await deleteSetting(db, key);
      if (!deleted) {
        throw new NotFoundError("Setting", key);
      }

      await auditFromCtx(c, {
        action: "setting.deleted",
        resourceType: "setting",
        resourceId: key,
        resourceName: key,
        result: "success",
      });

      return c.json({ success: true, data: null });
    },
  );

  return router;
}
