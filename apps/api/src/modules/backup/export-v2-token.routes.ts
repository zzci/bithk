...
export function backupExportV2TokenRoutes() {
  ...
  router.post("/backup/v2/exports-via-token", serviceTokenRequired("backup"), async (c) => {
    const db = c.get("db");
  ...
  router.get("/backup/v2/exports/:jobId/status-via-token", serviceTokenRequired("backup"), (c) => {
  ...
  router.get("/backup/v2/exports/:jobId/download-via-token", serviceTokenRequired("backup"), async (c) => {
    const jobId = c.req.param("jobId");
    const job = getExportJob(jobId);
    if (!job || job.ownerBucket !== bucketOf(c))
      throw new NotFoundError("Export job", jobId);
