...
import { finalizeReleasedBlob, listReferencesByOwner, releaseReferenceTx } from "@/modules/file";
...
export async function deleteComment(db: AppDatabase, commentId: string): Promise<void> {
  const refs = await listReferencesByOwner(db, COMMENT_ATTACHMENT_OWNER_TYPE, commentId);

  const drained = db.transaction((tx) => {
    const blobs = refs.map(r => releaseReferenceTx(tx, r.id));
