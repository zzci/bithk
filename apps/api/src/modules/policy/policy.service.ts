...
function checkDuplicateTuple(db: TxOrDb, input: CreateTupleInput): void {
...
export async function createTuple(db: AppDatabase, input: CreateTupleInput, createdBy: string): Promise<RelationTuple> {
  ...
  // BEGIN takes the write lock at transaction start, so the duplicate check +
  // insert serialize against concurrent writers — the only protection for NULL
  // subjectRelation rows, which idx_tuples_unique cannot enforce (SQLite
  // treats NULLs as distinct). Sync callback: see bun:sqlite transaction note.
  db.transaction((tx) => {
...
export async function batchCreateTuples(db: AppDatabase, inputs: readonly CreateTupleInput[], createdBy: string): Promise<readonly RelationTuple[]> {
  ...
  db.transaction((tx) => {
    // Dedup check inside the tx so it serializes with the inserts against
    // concurrent writers.
    for (const input of inputs) {
      checkDuplicateTuple(tx, input);
    }
