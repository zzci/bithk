...
describe("Policy Service", () => {
  ...
  describe("updateTupleRelation", () => {
    it("should replace the relation and drop the old row", async () => {
      const original = await createTuple(db, {
        namespace: "app",
        objectId: "my-app",
        relation: "viewer",
        subjectNamespace: "user",
      ...
      const updated = await updateTupleRelation(db, original.id, {
        namespace: "app",
        objectId: "my-app",
        relation: "admin",
        subjectNamespace: "user",
      ...
      expect(updated.relation).toBe("admin");
      expect(updated.id).not.toBe(original.id);
      // Old row gone, exactly one tuple left with the new relation.
      const tuples = await getTuplesByObject(db, "app", "my-app");
      expect(tuples).toHaveLength(1);
      expect(tuples[0]!.relation).toBe("admin");
    });

    it("should preserve the original grant when the new relation is invalid", async () => {
      const original = await createTuple(db, {
        namespace: "app",
        objectId: "my-app",
        relation: "viewer",
      ...
      await expect(
        updateTupleRelation(db, original.id, {
          namespace: "app",
          objectId: "my-app",
          relation: "invalid_rel",
          subjectNamespace: "user",
      ...
      // Validation throws before any write — the grant survives untouched.
      const tuples = await getTuplesByObject(db, "app", "my-app");
      expect(tuples).toHaveLength(1);
      expect(tuples[0]!.id).toBe(original.id);
      expect(tuples[0]!.relation).toBe("viewer");
    });

    it("should roll back (keep the original) when the target relation duplicates an existing tuple", async () => {
      const viewer = await createTuple(db, {
        namespace: "app",
        objectId: "my-app",
        relation: "viewer",
        subjectNamespace: "user",
      ...
      // Re-pointing the viewer tuple to "admin" would collide with the
      // existing admin tuple; the in-tx dup-check must abort and roll the
      // delete back so the viewer grant is not lost.
      await expect(
        updateTupleRelation(db, viewer.id, {
          namespace: "app",
          objectId: "my-app",
          relation: "admin",
          subjectNamespace: "user",
      ...
      ).rejects.toThrow("Duplicate tuple");

      const tuples = await getTuplesByObject(db, "app", "my-app");
      expect(tuples).toHaveLength(2);
      expect(tuples.some(t => t.id === viewer.id && t.relation === "viewer")).toBe(true);
    });
  });

  describe("addGroupMembership", () => {
    it("should insert once and be idempotent on a repeat add", async () => {
      const groupId = "dev-team";

      expect(await addGroupMembership(db, groupId, "u1", userId)).toBe(true);
      // Second add of the same membership is a no-op (returns false), and must
      // not create a duplicate NULL-subjectRelation row.
      expect(await addGroupMembership(db, groupId, "u1", userId)).toBe(false);

      const tuples = await getTuplesByObject(db, "group", groupId);
      expect(tuples).toHaveLength(1);
      expect(tuples[0]!.relation).toBe("member");
      expect(tuples[0]!.subjectId).toBe("u1");
    });
  });
});
