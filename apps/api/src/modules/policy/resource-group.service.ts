...
export async function createResourceGroup(
  ...
  // keys on objectId, so it offers no name-level backstop). Sync callback:
  // bun:sqlite transactions are synchronous.
  db.transaction((tx) => {
    const existing = tx
      .select()
      .from(relationTuples)
...
export async function deleteResourceGroup(db: AppDatabase, id: string): Promise<boolean> {
  ...
  db.transaction((tx) => {
    // Delete meta tuple
    tx.delete(relationTuples).where(eq(relationTuples.id, meta.id)).run();

    // Delete all member tuples (<resource>:<id>#parent@resource_group:<id>)
    tx.delete(relationTuples).where(
      and(
