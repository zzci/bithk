...
import { ShipProjectBindDialog } from "./-ship-project-bind-dialog";
...
export function ShipProjectsTab({ ship, canManage }: ShipProjectsTabProps) {
  ...
  // Pull the base project to the top; the rest keep their server order (stable).
  const sorted = [...projects].sort((a, b) => Number(b.isBase) - Number(a.isBase));
  ...
  return (
    <div className="space-y-4">
      ...
      {canManage && (
        <ShipProjectBindDialog
          shipId={ship.id}
          boundProjectIds={projects.map(p => p.id)}
          open={pickerOpen}
