...
const project = { id: "p1", name: "Serenity", sections: ["worklist"] } as unknown as ProjectView;
...
function routeFetch() {
  fetchMock.mockImplementation(async (input, init) => {
    ...
    if (method === "GET" && path.startsWith("/projects/p1/worklists"))
    ...
    if (method === "POST" && path === "/projects/p1/worklists")
      return jsonResponse({ success: true, data: { ...worklist, id: "wl2", name: "Hull check" } });
    return new Response("not found", { status: 404 });
  });
}

describe("projectWorklistTab", () => {
  it("renders worklists with their tags", async () => {
    routeFetch();
    renderWithProviders(<ProjectWorklistTab project={project} canManage />);
  ...
  it("filters worklists by tag via the ListFilter", async () => {
    ...
    await waitFor(() => {
      expect(fetchMock.mock.calls.some(c =>
        String(c[0]).includes("/projects/p1/worklists?") && String(c[0]).includes("tagId=t1"))).toBe(true);
    });
  });
  ...
  it("submits the edited form via the create path without fromGlobalId", async () => {
    ...
    await waitFor(() => {
      const post = fetchMock.mock.calls.find(c => String(c[0]) === "/api/projects/p1/worklists" && c[1]?.method === "POST");
  ...
  it("creates a ship worklist from scratch", async () => {
    ...
    await waitFor(() => {
      const post = fetchMock.mock.calls.find(call => String(call[0]) === "/api/projects/p1/worklists" && call[1]?.method === "POST");
      expect(post).toBeDefined();
