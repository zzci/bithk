...
/**
 * Resolve every call with a FRESH Response. `mockResolvedValue` hands the same
 * instance to each call, and a body can only be read once.
 */
...
describe("ship-profile section hooks", () => {
  it("reads and writes the profile on the project-scoped route", async () => {
    respondWith(({ success: true, data: { hullNumber: "H-1", shipStatus: "active" } }));
...
describe("equipment section hooks", () => {
  it("lists, creates, updates, and deletes equipment on project-scoped routes", async () => {
    respondWith(({ success: true, data: [{ id: "eq1", name: "Generator" }] }));
    ...
    respondWith(({ success: true, data: { id: "eq2", name: "Pump" } }));
    ...
    respondWith(({ success: true, data: { id: "eq2", name: "Pump 2" } }));
  ...
  it("keeps the category vocabulary on the project-scoped route", async () => {
    respondWith(({ success: true, data: [{ id: "c1", nameZh: "Main engine (zh)", nameEn: "Main engine" }] }));
    ...
    respondWith(({ success: true, data: { id: "c2", nameZh: "Pump (zh)", nameEn: "Pump" } }));
...
describe("worklist section hooks", () => {
  it("fetches the worklist tag vocabulary from /tags?type=worklist", async () => {
    respondWith(({ success: true, data: [{ id: "t1", name: "Engine" }] }));
  ...
  it("lists a project's worklists and the global knowledge base", async () => {
    fetchMock.mockImplementationOnce(async () => jsonResponse({ success: true, data: [{ id: "wl1", name: "Quarterly", tags: [] }] }));
    ...
    fetchMock.mockImplementationOnce(async () => jsonResponse({ success: true, data: [{ id: "gw1", name: "Global", tags: [] }] }));
  ...
  it("coerces a tag-less worklist row to tags:[] at the boundary (regression: .map of undefined)", async () => {
    ...
    fetchMock.mockImplementationOnce(async () => jsonResponse({ success: true, data: [{ id: "wl1", name: "Quarterly" }] }));
    ...
    fetchMock.mockImplementationOnce(async () => jsonResponse({ success: true, data: [{ id: "gw1", name: "Global" }] }));
    const globals = renderHook(() => useGlobalWorklists(true), { wrapper: makeWrapper() });
    await waitFor(() => expect(globals.result.current.isSuccess).toBe(true));
  ...
  it("creates worklists from scratch and from a global template", async () => {
    respondWith(({ success: true, data: { id: "wl1", name: "Quarterly", tags: [] } }));
  ...
  it("updates and deletes on the project-scoped worklist route", async () => {
    respondWith(({ success: true, data: { id: "wl1", name: "Renamed", tags: [] } }));
  ...
  it("posts global templates to the admin /worklists route", async () => {
    respondWith(({ success: true, data: { id: "gw1", name: "Template", tags: [] } }));
