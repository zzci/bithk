// Shared, presentational data shapes for file/drive surfaces.
//
// The drive file-list surface is presentational and renders `DisplayItem`s.
// Consumers fetch `DriveEntry`s from the API layer (`@/shared/lib/api/drive`)
// and bridge them with `entryToDisplayItem`. Keeping the adapter here means
// every consumer maps entries identically and the surface never depends on the
// API client. Byte formatting routes through the canonical `formatBytes`.
